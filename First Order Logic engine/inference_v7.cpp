...
class Program
{
	...
	/*static void isRuleAlreadyExisting(Rule rule)
	{
		...
		{
			for(vector<Rule>::iterator r = Program::rules[rule.right.name].begin();r!=Program::rules.end();r++)
			{
				if(!checkForVisitedPredicate((*r).right,rule.right))
					continue;
				if(rule.left_predicate_count != (*r).left_predicate_count)
					continue;
				bool isSame = true;
				for(int i = 0; i < rule.left_predicate_count;i++)
					if(!checkForVisitedPredicate(rule.left[i],(*r).left[i]))
					{
						isSame = false;
						break;
					}	
