...
void parsePredicate(string line, ofstream &myLogfile, bool isLogging)
{
	...
	smatch fact_match;
	smatch rule_match;
	if (regex_search(line, fact_match, factrgx))
	{
...
Rule unify(Rule r, Predicate pred, ofstream &myLogfile)
{
	Rule rule = r;
	Predicate pred_to_be_unified;
	//Right hand predicate is unified
	if(r.right.name.compare(pred.name)==0)
	{
	...
	if(Program::rules.find(pred.name)!=Program::rules.end())
	{
		for(vector<Rule>::iterator it = Program::rules[pred.name].begin();it!=Program::rules[pred.name].end();it++)
		{
			if((*it).right.isNegated==pred.isNegated && checkRHS((*it).right,pred))
			{
				...
				Rule r = unify((*it),pred,myLogfile);
				queue<QueueNode> q;
				QueueNode node;
				node.r = r;
				node.level = 0;
				...
				while(!q.empty())
				{
					...
					if(i<(*it).left_predicate_count)
					{
						...
						if(assigned_backtrack.isFound)
						{
							...
							{
								Rule new_rule = unify(front.r,(*iter),myLogfile);
								if(isLogging)
								{
				...
				if(istrue)
				{
					PredicateMatchResult predicate;
					vector<Predicate> preds;
					while(!q.empty())
					{
						QueueNode node = q.front();
					...
					predicate.preds = preds;
					predicate.isFound = true;
					if(isLogging)
					log(whitespace+"value returned : "+to_string(predicate.isFound)+"\n",myLogfile);
					return predicate;
				}	
			}
		}
