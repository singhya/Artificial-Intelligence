...
UnifyStruct unify(Rule r, Predicate pred, ofstream &myLogfile, bool isRight,int left_pred_index, string whitespace, bool is_logging)
{
	...
	int left_pred_index_calculated = 0;
	...
	else
	{
		for(int i = left_pred_index;i<r.left_predicate_count;i++)
		{
			if(r.left[i].name.compare(pred.name)==0)
			{
				...
				left_pred_index_calculated = i;
				break;
			}
		}
	}
	Predicate pred_to_be_unified_for_logging = pred_to_be_unified;
	map<string,string> local_map;
	for(int i = 0; i<pred.attribute_count;i++)
	{
		if(pred.attributes[i][0]>='a' && pred.attributes[i][0]<='z')
		{
			if(pred_to_be_unified.attributes[i][0]>='a' && pred_to_be_unified.attributes[i][0]<='z')
			{
				if(local_map.find(pred.attributes[i])!=local_map.end())
				{
					rule.substitution[pred_to_be_unified.attributes[i]]  = local_map[pred.attributes[i]];
					rule.substitute();
					if(rule.substitution[pred_to_be_unified.attributes[i]][0]>='a' && 
						rule.substitution[pred_to_be_unified.attributes[i]][0]<='z')
					rule.substitution[pred_to_be_unified.attributes[i]]="";
					if(isRight)
					{
						pred_to_be_unified = rule.right;
					}
					else
					{
						pred_to_be_unified = rule.left[left_pred_index_calculated];
					}
					//Update pred_to_be_unified
				}
				else
				{
					local_map[pred.attributes[i]] = pred_to_be_unified.attributes[i];
				}
			}
			else
			{
		...
		else if(pred_to_be_unified.attributes[i][0]>='a' && pred_to_be_unified.attributes[i][0]<='z')
		{
		...
		else if(pred_to_be_unified.attributes[i].compare(pred.attributes[i])!=0)
		{
			result.r = rule;
			result.can_be_unified=false;
			if(is_logging)
			{
				log(whitespace+"Can't unify the given below predicates : \n",myLogfile);
				pred.displayFact(whitespace,myLogfile);
				log("\n",myLogfile);
				pred_to_be_unified_for_logging.displayFact(whitespace,myLogfile);
				log("\n",myLogfile);
			}
			return result;
		}	
	...
	if(is_logging)
	{
		log("\n"+whitespace+"Local Mapping\n",myLogfile);
		string output="";
		for(map<string,string>::iterator i = local_map.begin();i!=local_map.end();i++)
		{
			output+=whitespace+i->first;
			output+=" : ";
			output+=i->second;
			output+="\n";
		}
