...
Rule unify(Rule r, Predicate pred, ofstream &myLogfile, bool isRight,int left_pred_index)
{
	Rule rule = r;
	...
	if(Program::rules.find(pred.name)!=Program::rules.end())
	{
		for(vector<Rule>::iterator it = Program::rules[pred.name].begin();it!=Program::rules[pred.name].end();it++)
		{
			if((*it).right.isNegated==pred.isNegated && checkRHS((*it).right,pred))
			{
				...
				Rule r = unify((*it),pred,myLogfile,true,0);
				queue<QueueNode> q;
				QueueNode node;
				node.r = r;
				node.level = 0;
				...
				while(!q.empty())
				{
					...
					if(i<(*it).left_predicate_count)
					{
						...
						if(assigned_backtrack.isFound)
						{
							...
							{
								Rule new_rule = unify(front.r,(*iter),myLogfile,false,front.level);
								if(isLogging)
								{
