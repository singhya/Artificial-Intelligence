...
state repeatEvaluationForMin(state s, int cutoff, int player,int strategy)
{
	...
	for(int i=0;i<N;i++)
	{
		if((player==1 && s.player1[i]!=0) || (player==2 && s.player2[i]!=0))
		{
			...
			{
				branches[i].s.alpha = branches[i].s.mancala1-branches[i].s.mancala2;
			}
			branches[i].s.value = branches[i].s.alpha;
			generateLog("Min",branches[i].s,strategy);
			if(branches[i].repeat)
			{
				output = repeatEvaluationForMin(branches[i].s, cutoff, player, strategy).value;
			}
			else
			{
				output = MaxValue(branches[i].s, cutoff, player%2+1, strategy).value;
			}
			if(output<s.beta || output<s.value)
			{	
				s.value = output;
				s.beta = s.value;
			}
...
state repeatEvaluationForMax(state s, int cutoff, int player, int strategy)
{
	...
	for(int i=0;i<N;i++)
	{
		if((player==1 && s.player1[i]!=0) || (player==2 && s.player2[i]!=0))
		{
			...
			if(branches[i].s.depth==cutoff)
			{
				branches[i].s.beta = branches[i].s.mancala1-branches[i].s.mancala2;
			}
			branches[i].s.value = branches[i].s.beta;
			...
			if(output>s.alpha || output>s.value)
			{	
				s.value = output;
				s.alpha = s.value;
				final_state = next_state;
				//log("Min",operation,depth,player,alpha,beta);
			}	
			s.value = s.alpha;
			operations.pop();
